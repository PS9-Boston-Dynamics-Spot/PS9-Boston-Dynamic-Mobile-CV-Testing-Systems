...
class DatabaseWriter(SqliteConnector):
    ...
    def insert_raw_image(self, name: str, format: str, bucket: str, size: int, compressed: bool, compression_method: str) -> tuple:
        query = """
        ...
        try:
            with self.connector as cursor:
                cursor.execute(query, (name, format, bucket, size, compressed, compression_method))
