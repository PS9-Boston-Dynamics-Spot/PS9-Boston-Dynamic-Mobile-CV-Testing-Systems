...
from key_point_extraction import (
...
class KeyPointVal:
    def __init__(self, model, base_path, time_str=None):
        ...
        self.time_str = (
            time_str if time_str is not None else time.strftime("%Y%m%d-%H%M%S")
        )

        train_image_folder = os.path.join(base_path, TRAIN_PATH, IMG_PATH)
        train_annotation_folder = os.path.join(base_path, TRAIN_PATH, LABEL_PATH)
    ...
    def validate_set(self, path, dataset):
        ...
        for index, data in enumerate(dataset):
            ...
            # Extract key points
            key_points_predicted = full_key_point_extraction(heatmaps, threshold=0.6)
            ...
            key_point_metrics_dict[image_name][START_END_KEY] = key_point_metrics(
                np.vstack((key_points_predicted[0], key_points_predicted[2])),
            ...
            original_image_tensor = custom_transforms(train=False, image=original_image)
            plot_key_points(
                original_image_tensor,
                key_points_predicted,
                key_points_true,
                key_point_file_path,
            )
...
def evaluate_total_metrics(metrics_dict, key):
    ...
    for single_metrics_dict in metrics_dict.values():
        ...
        total_non_assigned += single_metrics_dict[key][NON_ASSIGNED_KEY] / n_tests
    full_metrics_dict = {
...
def plot_heatmaps(heatmaps1, heatmaps2, filename):
    ...
    if N_HEATMAPS == 1:
        ...
        heatmap_plot = plt.imshow(
            heatmaps1.squeeze(0), cmap=plt.cm.viridis, vmin=0, vmax=1
        )
        ...
        plt.subplot(2, 1, 2)
        heatmap_plot = plt.imshow(
            heatmaps2.squeeze(0), cmap=plt.cm.viridis, vmin=0, vmax=1
        )
        plt.colorbar(heatmap_plot, shrink=0.5)
        ...
        plt.subplots_adjust(
            wspace=0.2, hspace=0, left=0.1, right=0.9, top=0.9, bottom=0.1
        )
        for i in range(3):
            im = axs[0, i].imshow(heatmaps1[i], cmap=plt.cm.viridis, vmin=0, vmax=1)
            axs[0, i].set_title(f"Predicted Heatmap {titles[i]}")
        im1 = im
        for i in range(3):
            im = axs[1, i].imshow(heatmaps2[i], cmap=plt.cm.viridis, vmin=0, vmax=1)
